    ...
    if (!FindRegisterWithContents(var, reg)) {
        ...
        if (reason == ForRead) {                 // load current value
            ...
            Emit("lw %s, %d(%s)\t# load %s from %s%+d into %s", regs[reg].name,
                 var->GetOffset(), offsetFromWhere, var->GetName().c_str(),
...
void Mips::SpillRegister(Register reg)
{
    ...
    if (var && regs[reg].isDirty) {
        ...
        Emit("sw %s, %d(%s)\t# spill %s from %s to %s%+d", regs[reg].name,
             var->GetOffset(), offsetFromWhere, var->GetName().c_str(), regs[reg].name,
...
void Mips::EmitLoadAddress(Location *dst, Location *ref) {
    Register reg = GetRegisterForWrite(dst);
    if (ref->GetSegment() == fpRelative)
        Emit("la %s, %d($fp)", regs[reg].name, ref->GetOffset());
    else
        Emit("la %s, %d($gp)", regs[reg].name, ref->GetOffset());
}
